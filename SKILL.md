...
description: Agent记忆系统搭建+行为进化引擎。从零搭建Agent长期记忆系统，并实现记忆到行为的闭环：纠正捕获→模式检测→规则提炼→指针验证→记忆压缩。
...
# Agent 记忆觉醒与行为进化 Skill

> 🧠 从零搭建 Agent 长期记忆系统，并让记忆真正驱动行为改变。
>
> **Part 1 · 记忆系统搭建**：覆盖本地优先的 `MEMORY.md`、每日笔记、`SESSION-STATE.md`、`working-buffer.md`、Obsidian 原生笔记与可选召回后端全流程。
>
> **Part 2 · 行为进化引擎**：从经验中自动提炼行为规则，实现"记忆→行为"闭环——纠正捕获、模式检测、规则提炼、指针验证、记忆压缩。

## 触发词

`记忆系统`、`memory-setup`、`搭建记忆`、`记忆架构`、`行为进化`、`纠正捕获`、`记忆压缩`、`指针验证`、`行为回顾`
...
> 🧬 记忆不是终点，行为改变才是。Part 1 解决了"怎么记"，Part 2 解决"记了之后怎么变"。
>
> 当前所有 Agent 记忆系统的共同缺陷：**记忆只影响"下次搜到什么"，不影响"下次怎么做"**。行为进化引擎要解决的就是这个问题——让纠正不只是记下来，而是直接写入行为规则；让反复踩坑不只是回忆起来，而是自动生成防护规则。

## 触发场景

### 场景 1：行为纠正捕获（实时）
用户明确纠正 Agent 行为时触发：
- "以后别这样做""记住这个教训""别再犯这种错"
- "你上次就错了，怎么又……"

### 场景 2：模式检测（周期性）
Agent 定期回顾积累的记忆，检测重复模式：
- 同一类错误出现 2 次以上
- 用户反复纠正同一件事
- 某个决策模式持续导致不良结果
...
```
用户纠正 → ① 记录纠正事件 → ② 评估影响级别 → ③ 决定写入位置 → ④ 执行写入 → ⑤ 确认生效 → ⑥ 维护强制检索规则表
```

**① 记录纠正事件**

将纠正事件写入 `corrections/` 目录下日志文件：

```markdown
## [日期] [来源]
- **触发情境**：用户说了什么 / 做了什么
- **Agent行为**：Agent 当时做了什么
- **用户纠正**：用户期望 Agent 怎么做
- **影响级别**：P0(必须立即改) / P1(重要) / P2(优化)
- **关联模式**：（如果检测到类似纠正已有记录，标注关联）
```

文件命名：`corrections/YYYY-MM.md`（按月归档）

**② 评估影响级别**

| 级别 | 判断标准 | 写入位置 |
|------|---------|---------|
| P0 | 用户明确说"以后必须/永远不要" | SOUL.md 或 MEMORY.md 长期行为规则 |
| P1 | 反复纠正(≥2次)同一类行为 | MEMORY.md 长期行为规则 或 TOOLS.md |
| P2 | 单次纠正，影响较小 | corrections/ 日志，暂不写入规则 |

**③ 决定写入位置**

| 纠正类型 | 写入目标 | 示例 |
|---------|---------|------|
| 身份/性格/风格 | SOUL.md | "说话不要太啰嗦" |
| 通用行为规则 | MEMORY.md `## 长期行为规则` | "发消息前必须先读取上下文文件" |
| 工具/平台操作经验 | TOOLS.md | "用XX工具时要注意参数格式" |
| 用户个人偏好 | USER.md | "用户不喜欢被叫大名" |

**④ 执行写入**

- 使用 `edit_file` 的 `append` 或 `replace_one` 模式
- 写入前先读取目标文件，确保不覆盖现有规则
- 如果目标文件已存在类似规则，更新而非重复添加
- 每条规则格式统一：`- **简短标题**：具体规则内容`

**⑤ 确认生效**

写入后向用户确认：写入了什么规则、写到了哪个文件、下次遇到类似场景会怎么做。

**⑥ 维护强制检索规则表（关键！）**

如果新规则涉及"遇到特定场景必须先读取某个文件"，必须同步更新 MEMORY.md 的 `## ⚠️ 强制检索规则` 表：

```markdown
| 触发关键词 | 必须读取文件 | 说明 |
|---|---|---|
| 关键词1/关键词2 | `文件路径` | 为什么必须读 |
```

**维护规则：**
- 新写入的规则如果涉及"先读 xxx 再操作"→ 必须在表中新增一行
- 规则被修改或删除 → 同步更新表中对应行
- 表的触发关键词要尽量全面，覆盖用户可能的各种表述
...
1. 读取 `corrections/` 目录下最近 30 天的纠正记录
2. 按"触发情境"和"纠正内容"进行语义聚类
3. 对每个聚类：
   - 出现 ≥ 2 次 → 标记为"重复模式"
   - 出现 ≥ 3 次 → 标记为"高频模式"，建议提升为规则
4. 对"高频模式"生成规则建议：提炼为一句话规则 + 推荐写入位置 + 展示给用户确认
...
1. **扫描现有规则**：读取 SOUL.md、MEMORY.md、TOOLS.md 中的所有行为规则
2. **对照近期经验**：读取 corrections/、recent_memory/、用 memory_search 搜索相关对话
3. **生成三类建议**：

| 建议类型 | 判断标准 | 示例 |
|---------|---------|------|
| 新增规则 | 反复出现的纠正/教训，尚未形成规则 | "每次发消息都要先读上下文" |
| 修改规则 | 现有规则与实际行为冲突，或不够具体 | 原规则"提醒用户喝水"→ 修改为"仅在10:00-22:00提醒" |
| 删除规则 | 长期未被触发，或已过时 | 某个已不再使用的工具的注意事项 |
...
1. **评估当前状态**：检查是否已有 SOUL.md、MEMORY.md、TOOLS.md 等配置文件和 corrections/ 目录
2. **搭建基础结构**：创建 `corrections/` 目录和 `corrections/index.json`
3. **从历史记忆中提取规则**：用 memory_search 搜索"纠正""错误""教训"等关键词，提取隐含行为规则
4. **写入初始规则**：将提取的规则写入对应配置文件，向用户展示完整清单

---

## 工作流 E：指针验证（定期）

目的：确保 MEMORY.md 中强制检索规则表里的所有文件路径都真实存在、内容未过时。

**执行步骤：**

1. **扫描强制检索表**：读取 MEMORY.md 中 `## ⚠️ 强制检索规则` 表的所有行
2. **验证文件存在性**：检查每个路径指向的文件是否存在
3. **检查内容新鲜度**：对比文件最后修改时间，标记过期内容
4. **生成验证报告**：
...
| 触发词 | 目标文件 | 状态 |
|---|---|---|
| 持仓/止损 | 投资/炒股档案/持仓_latest.json | ✅ 存在 |
| 旧项目 | recent_memory/project/已删除.md | ❌ 文件不存在 |
...
5. **自动修复**（需用户确认）：删除指向不存在文件的行，更新过期的文件路径

可通过 Calendar 设置每周自动执行一次，或集成到工作流 C 的定期回顾中。

---

## 工作流 F：记忆压缩（定期）

目的：保持 MEMORY.md 精简高效，防止超过 5120 字节上限，同时确保指针指向有效文件。

**执行步骤：**

1. **运行指针验证**：执行 `verify_pointers --workspace .`，修复所有失效指针
2. **清理过时状态锚点**：
   - 已结束的临时事件 → 删除或压缩为一行
   - 仍在进行中的项目 → 保留但精简措辞
   - 长期有效的状态 → 保留
3. **清理过时行为规则**：已不再适用的规则删除，重复的规则合并
4. **压缩技巧**：
   - 多条相似状态合并为一条
   - 详细内容移到 `recent_memory/`，MEMORY.md 只留摘要 + 指针
   - 指针必须同步更新到强制检索规则表
5. **验证压缩结果**：确保 MEMORY.md 大小在 4500 字节以下

**压缩频率**：每月一次，或 MEMORY.md 超过 4800 字节时触发。
...
python3 scripts/behavior_evolution.py init --workspace /path/to/workspace

# 分析纠正记录，检测重复模式
python3 scripts/behavior_evolution.py analyze --workspace /path/to/workspace

# 基于检测结果生成规则建议
python3 scripts/behavior_evolution.py suggest --workspace /path/to/workspace

# 生成行为回顾报告
python3 scripts/behavior_evolution.py report --workspace /path/to/workspace

# 验证 MEMORY.md 中所有指针的有效性
python3 scripts/behavior_evolution.py verify_pointers --workspace /path/to/workspace
```

### memory_capture.py（Part 1 配套脚本）
...
# 任务结束记忆捕获蒸馏
python3 scripts/memory_capture.py distill --workspace /path/to/workspace

# 将蒸馏结果写入 MEMORY.md
python3 scripts/memory_capture.py apply --workspace /path/to/workspace

# 维护报告（只读，不写入）
python3 scripts/memory_capture.py report --workspace /path/to/workspace
...
```markdown
## 长期行为规则
- **规则标题**：具体规则描述（来源：YYYY-MM-DD 用户纠正/经验教训）
```

### TOOLS.md 中的操作经验
```markdown
## 工具操作经验
- **工具名/场景**：具体注意事项（来源：YYYY-MM-DD 踩坑记录）
```

### corrections/index.json 格式
```json
{
  "corrections": [
    {
      "id": "c_001",
      "date": "2026-07-23",
      "trigger": "用户纠正Agent发消息不带背景",
      "agent_action": "直接发送结果，未附带上下文",
      "user_correction": "每次发消息都要先读取上下文文件",
      "severity": "P0",
      "pattern": "消息发送规范",
      "rule_applied": true,
      "rule_target": "MEMORY.md",
      "rule_text": "微信推送前必须先读取上下文文件"
    }
  ],
  "patterns": {
    "消息发送规范": {
...
1. **不自动删除规则**：删除建议必须经用户确认后才执行
2. **不覆盖身份定义**：SOUL.md 的身份部分只能通过用户明确要求修改
3. **不修改其他 Agent 的配置**：只操作当前 Agent 的配置文件
4. **变更可追溯**：每次规则变更记录来源日期和原因
5. **保守写入**：P2 级别的纠正只记录不写入规则，除非反复出现
...
**一句话总结**：Part 1 让 Agent "记得住"，Part 2 让 Agent "改得了"。两者合一，才是真正的记忆觉醒。
...
| Agent 类型 | 建议回顾频率 | 说明 |
|-----------|------------|------|
| 个人助理 | 每周一次 | 纠正频率高，需要快速迭代 |
| 工作协作 | 每两周一次 | 规则相对稳定 |
| 专用工具 | 每月一次 | 变化少，重稳定性 |

可通过 Calendar 设置定期提醒，或在 Heartbeat 中增加检查项。
